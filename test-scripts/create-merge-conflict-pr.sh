...
_branch="security-patch-testing-conflict/$(date '+%Y%m%d%H%M%S')"
...
echo "This is a conflicting commit $(date)" >> newFile-security-patches
git add . && git status
git commit -m "Adding conflicting commit right around  $(date)"
